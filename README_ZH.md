...
- **🌐 跨语言匹配** — 中文查询开箱即用，通过 `data/term-map.json` 同义词映射
- **🔗 技能串联** — 自动将多个技能串联执行复杂任务，含上下文传递规则和错误降级
- **📊 调用追踪** — 记录每次调用（成功/失败）用于学习
- **🧠 模式学习** — 检测重复的技能序列，建议创建组合技能
- **⚡ 增量索引** — 仅重新解析内容变化的技能文件，基于 SHA256 hash 缓存
- **🧪 测试套件** — 38 个自动化测试覆盖核心函数
...
**跨语言支持：** 中文查询通过 `data/term-map.json` 自动翻译为英文关键词后参与评分。
中英混合查询同样有效。
...
├── data/                        # 持久化状态（自动生成）
│   ├── skill-index.json         # 技能目录 + 统计 + 文件 hash
│   ├── invocation-history.json  # 调用日志（最近 500 条）
│   ├── learned-patterns.json    # 检测到的使用模式
│   └── term-map.json            # 中英文同义词映射
├── tests/
│   └── test_dreamhive.py        # 自动化测试套件（38 个测试）
├── LICENSE                      # MIT 许可证
└── README.md                    # 本文档
```
